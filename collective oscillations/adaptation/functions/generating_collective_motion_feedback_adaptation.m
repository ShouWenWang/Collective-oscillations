function [time,h,x_bar,heat_rate_h]=generating_collective_motion_feedback_adaptation(T0,dt,N,para_array,wh_nonlinear,wx_nonlinear,plot_on)
...
for j=2:L
    ...
    h(j)=h(j-1)+basal*dt-wh(j-1)*h(j-1)*dt+alpha_1(j-1)*N*x_bar(j-1)/gamma_h*dt+noise_h(j-1)-wh_nonlinear*h(j-1)^3*dt;
    x(j,:)=x(j-1,:)-wx*(x(j-1,:)-y(j-1,:))*dt+alpha_2(j-1)*h(j-1)/gamma_x*dt+noise_x(j-1,:)-wx_nonlinear*x(j-1,:).^3*dt;
...
heat_rate_h=alpha_1*mean(Qh,2)/T0;
