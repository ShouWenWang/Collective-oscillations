...
%    h(j)=h(j-1)+N*beta_2*dt+N*alpha_1*(aver_x(j-1)-station_activity)*dt-(wh+N*beta_0)*h(j-1)*dt-h(j-1)^3*dt;%+noise_h(j-1);%-h(j-1)^3*dt;
%% linear signal
    h(j)=h(j-1)+N*beta_2*dt+N*alpha_1*(aver_x(j-1)-station_activity)*dt-(wh+N*beta_0)*h(j-1)*dt;%+noise_h(j-1);%-h(j-1)^3*dt; %noise cannot be added, otherwise negative signal emerges, bad for log
%% linear signal with no density dependent bias, still has decrease oscillation frequency
%    h(j)=h(j-1)+N*beta_2*dt+N*alpha_1*aver_x(j-1)*dt-0.9*station_activity*dt-(wh+N*beta_0)*h(j-1)*dt;%+noise_h(j-1);%-h(j-1)^3*dt; %noise cannot be added, otherwise negative signal emerges, bad for log
