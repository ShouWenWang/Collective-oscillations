...
%% temporal collective dynamics
T0=300; % duration
dt=0.01; 
epsilon=0.1;
plot_on=1;
effective_N=1.5; %0.5, 0.85, 1.5
[h,aver_x,heat_rate_h]=generating_oscillation_FHN(T0,dt,epsilon,effective_N,plot_on);
...
%% original Dicty model 

rho=[10^(-7) 10^(-6) 5*10^(-6) 10^(-5) 2*10^(-5) 3*10^(-5) 4*10^(-5) 5*10^(-5) 6*10^(-5)  10.^(-4) 10^(-3) 5*10^(-3) 10^(-2) 5*10^(-2) 0.1];

%rho=[10.^(-4) 10^(-3) 5*10^(-3) 10^(-2) 5*10^(-2) 0.1];

for k=1:length(rho)
[h,aver_x_new]=generating_oscillation_cAMP(rho(k));

data_simu_dicty{k,1}=h;
data_simu_dicty{k,2}=aver_x_new;

end


alpha0=800; alpha_PDE=10^3;
x0=-1.5;
S=10^6;  J=1;
h=@(x,rho) rho*alpha0/(J+alpha_PDE)+rho*S/(J+alpha_PDE*rho)*x;
rho=10^(-7)*2.^(1:30);
figure, semilogx(rho,h(x0,rho),'-r');
xlim([10^(-7) 1]);


figure, semilogx(rho,rho*S./(J+alpha_PDE*rho),'-r');

a=0.058;  K0=10^(-5);
figure, semilogx(rho,a*log(1+h(2,rho)/K0),'-r');
%xlim([10^(-7) 1]);
 ...
 c0=1.2;  r=0.5; 
f=@(x,input) x-x.^3/3+input;

g=@(x) 1/r*(x+c0);
x=-2:0.01:1;
figure, plot(x,f(x,1),'-r',x,g(x),'-g');
xlabel('x');
%ylabel('$y$');
...
cutoff_time=400;
k=4;%data_simu_dicty{M,1}
[Ch_fre,Ch_velo,fre]=Correlation_spectrum(data_simu_dicty{k,2},dt,cutoff_time);


figureParameter
f1=plot(fre(2:end),Ch_fre(2:end),'-r');
a1=xlabel('$\omega$');
xlim([0.01 1]);
%ylim([0 1.2]);
h1=legend('$\tilde{C}_h$');
%fig_name='./figure/cAMP-oscillation-spectrum-h.eps';
figurePostTreat
