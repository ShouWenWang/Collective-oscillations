...
Delta_T=10; % the number of ignored early period for computing the phase and frequency
T0=length(x)*dt;
max_period=floor(T0/(2*pi/fre)-2);
if max_period<(Delta_T+1)
    error("please increase the duration")
end
%% the intermediate period between T1 and T2 for estimating the phase shift
T1=(max_period-Delta_T)*2*pi/fre; 
